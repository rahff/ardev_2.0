import { Body, Controller, Get, NotFoundException, Param, Post, Query, Req, Res, UseInterceptors } from '@nestjs/common';
...
export class AppController {
  ...
  async getCalendar(@Res() res: Response, 
                    @Query('mounth') mounthIndex: string,
                    @Query('year') yearIndex: string){
                      
    const calendarData = this.calendarService.getCalendar(mounthIndex, yearIndex);
    res.render('components/calendar', {calendar: calendarData});
  }
